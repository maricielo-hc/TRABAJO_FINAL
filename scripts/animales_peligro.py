...
def obtener_estado_conservacion(nombre_especie):
    ...
    title = link.get('title') if link and link.get('title') else ""
    ...
    for estado in estados_validos:
        if estado in texto or estado in title:
            return estado

    return "Estado no disponible"

# 📥 Leer archivo de entrada
input_path = "especies_nom_coord.json"  # Usa el nombre correcto según tu proyecto
...
estados_excluir = [
    "Preocupación menor", "Datos insuficientes", "Estado no identificado",
    "No se encontró sección", "No se pudo acceder (404)", "Estado no disponible"
]
especies_filtradas = [
